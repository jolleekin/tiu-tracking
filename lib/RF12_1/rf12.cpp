...
/* DDL: include this so that we can use arduino standard lib functions right here */
#include <WProgram.h>
...
/* DDL: The original file was .c file, but somehow, I need to change it to .cpp in order to be able to
build it using arduino IDE */

/* DDL: Receive signal strength */
long RSSI = 0;
long readRSSI()
{
	return RSSI;
...
{	unsigned char i;
	...
	{	rf12_ready();
		*data++=rf12_trans(0xB000);
		if(i == 1)
			RSSI = analogRead(A0); // DDL: sampling RSSI right here :)
	}
